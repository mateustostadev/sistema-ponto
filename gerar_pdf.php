...
function gerarRelatorioPDF($idFuncionario, $mes, $ano, $dadosPonto, $nomeFuncionario)
{
    ...
    $pdf->Cell(30, 10, 'Almoço', 1, 0, 'C');
    $pdf->Cell(30, 10, 'Retorno', 1, 0, 'C');
    $pdf->Cell(30, 10, 'Saída', 1, 1, 'C');

    foreach ($dadosPonto as $dados) {
        $pdf->Cell(30, 10, $dados['data'], 1, 0, 'C');
        $pdf->Cell(30, 10, $dados['entrada'], 1, 0, 'C');
        $pdf->Cell(30, 10, $dados['almoco'], 1, 0, 'C');
        $pdf->Cell(30, 10, $dados['retorno'], 1, 0, 'C');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $idFuncionario = isset($_POST['funcionario']) ? $_POST['funcionario'] : null;
    $dataInicial = isset($_POST['data_inicial']) ? $_POST['data_inicial'] : null;
    $dataFinal = isset($_POST['data_final']) ? $_POST['data_final'] : null;

    
    if (empty($idFuncionario) || empty($dataInicial) || empty($dataFinal)) {
    ...
    $sql = "SELECT usuario_id, data_hora_entrada, data_hora_almoco, data_hora_retorno, data_hora_saida
FROM tabela_ponto
...
ORDER BY data_hora_entrada";
    ...
    while ($row = $result->fetch_assoc()) {
        $dadosPonto[] = [
            ...
            'almoco' => date('H:i', strtotime($row['data_hora_almoco'])),
            'retorno' => date('H:i', strtotime($row['data_hora_retorno'])),
